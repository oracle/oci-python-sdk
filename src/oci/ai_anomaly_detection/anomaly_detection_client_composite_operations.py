...
class AnomalyDetectionClientCompositeOperations(object):
    ...
    def change_data_asset_compartment_and_wait_for_state(self, data_asset_id, change_data_asset_compartment_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        ...
        try:
            waiter_result = oci.wait_until(
                self.client,
                self.client.get_data_asset(data_asset_id),  # noqa: F821
    ...
    def create_detect_anomaly_job_and_wait_for_state(self, create_detect_anomaly_job_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.ai_anomaly_detection.AnomalyDetectionClient.create_detect_anomaly_job` and waits for the :py:class:`~oci.ai_anomaly_detection.models.DetectAnomalyJob` acted upon
        to enter the given state(s).
        ...
        :param list[str] wait_for_states:
            An array of states to wait on. These should be valid values for :py:attr:`~oci.ai_anomaly_detection.models.DetectAnomalyJob.lifecycle_state`

        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.ai_anomaly_detection.AnomalyDetectionClient.create_detect_anomaly_job`
        ...
        try:
            waiter_result = oci.wait_until(
                self.client,
                self.client.get_detect_anomaly_job(detect_anomaly_job_id),  # noqa: F821
                evaluate_response=lambda r: getattr(r.data, 'lifecycle_state') and getattr(r.data, 'lifecycle_state').lower() in lowered_wait_for_states,
                **waiter_kwargs
            )
            result_to_return = waiter_result
    ...
    def delete_data_asset_and_wait_for_state(self, data_asset_id, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        ...
        try:
            if ("succeed_on_not_found" in waiter_kwargs) and (waiter_kwargs["succeed_on_not_found"] is False):
                self.client.base_client.logger.warning("The waiter kwarg succeed_on_not_found was passed as False for the delete composite operation delete_data_asset, this would result in the operation to fail if the resource is not found! Please, do not pass this kwarg if this was not intended")
            else:
                """
    ...
    def delete_detect_anomaly_job_and_wait_for_state(self, detect_anomaly_job_id, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.ai_anomaly_detection.AnomalyDetectionClient.delete_detect_anomaly_job` and waits for the :py:class:`~oci.ai_anomaly_detection.models.DetectAnomalyJob` acted upon
        to enter the given state(s).
        ...
        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.ai_anomaly_detection.AnomalyDetectionClient.delete_detect_anomaly_job`

        :param dict waiter_kwargs:
            A dictionary of keyword arguments to pass to the :py:func:`oci.wait_until` function. For example, you could pass ``max_interval_seconds`` or ``max_interval_seconds``
            as dictionary keys to modify how long the waiter function will wait between retries and the maximum amount of time it will wait
        """
        initial_get_result = self.client.get_detect_anomaly_job(detect_anomaly_job_id)
        operation_result = None
        try:
            operation_result = self.client.delete_detect_anomaly_job(detect_anomaly_job_id, **operation_kwargs)
        except oci.exceptions.ServiceError as e:
            if e.status == 404:
                return WAIT_RESOURCE_NOT_FOUND
            else:
        ...
        try:
            if ("succeed_on_not_found" in waiter_kwargs) and (waiter_kwargs["succeed_on_not_found"] is False):
                self.client.base_client.logger.warning("The waiter kwarg succeed_on_not_found was passed as False for the delete composite operation delete_detect_anomaly_job, this would result in the operation to fail if the resource is not found! Please, do not pass this kwarg if this was not intended")
            else:
                """
                If the user does not send in this value, we set it to True by default.
                We are doing this because during a delete resource scenario and waiting on its state, the service can
    ...
    def update_detect_anomaly_job_and_wait_for_state(self, detect_anomaly_job_id, update_detect_anomaly_job_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.ai_anomaly_detection.AnomalyDetectionClient.update_detect_anomaly_job` and waits for the :py:class:`~oci.ai_anomaly_detection.models.DetectAnomalyJob` acted upon
        to enter the given state(s).
        ...
        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.ai_anomaly_detection.AnomalyDetectionClient.update_detect_anomaly_job`
