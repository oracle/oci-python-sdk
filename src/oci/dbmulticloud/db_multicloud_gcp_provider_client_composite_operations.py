...
class DbMulticloudGCPProviderClientCompositeOperations(object):
    ...
    def replicate_oracle_db_gcp_key_ring_and_wait_for_state(self, oracle_db_gcp_key_ring_id, replicate_oracle_db_gcp_key_ring_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.dbmulticloud.DbMulticloudGCPProviderClient.replicate_oracle_db_gcp_key_ring` and waits for the :py:class:`~oci.dbmulticloud.models.WorkRequest`
        to enter the given state(s).
        ...
        :param oci.dbmulticloud.models.ReplicateOracleDbGcpKeyRingDetails replicate_oracle_db_gcp_key_ring_details: (required)
        ...
        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.dbmulticloud.DbMulticloudGCPProviderClient.replicate_oracle_db_gcp_key_ring`

        :param dict waiter_kwargs:
            A dictionary of keyword arguments to pass to the :py:func:`oci.wait_until` function. For example, you could pass ``max_interval_seconds`` or ``max_interval_seconds``
            as dictionary keys to modify how long the waiter function will wait between retries and the maximum amount of time it will wait
        """
        operation_result = self.client.replicate_oracle_db_gcp_key_ring(oracle_db_gcp_key_ring_id, replicate_oracle_db_gcp_key_ring_details, **operation_kwargs)
        if not wait_for_states:
            return operation_result
        lowered_wait_for_states = [w.lower() for w in wait_for_states]
        if 'opc-work-request-id' not in operation_result.headers:
