...
class QuotasClientCompositeOperations(object):
    ...
    def add_quota_lock_and_wait_for_state(self, quota_id, add_lock_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.limits.QuotasClient.add_quota_lock` and waits for the :py:class:`~oci.limits.models.Quota` acted upon
        ...
        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.limits.QuotasClient.add_quota_lock`
        ...
        try:
            waiter_result = oci.wait_until(
                self.client,
                self.client.get_quota(quota_id),  # noqa: F821
    ...
    def delete_quota_and_wait_for_state(self, quota_id, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        ...
        try:
            if ("succeed_on_not_found" in waiter_kwargs) and (waiter_kwargs["succeed_on_not_found"] is False):
                self.client.base_client.logger.warning("The waiter kwarg succeed_on_not_found was passed as False for the delete composite operation delete_quota, this would result in the operation to fail if the resource is not found! Please, do not pass this kwarg if this was not intended")
            else:
                """
    ...
    def remove_quota_lock_and_wait_for_state(self, quota_id, remove_lock_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.limits.QuotasClient.remove_quota_lock` and waits for the :py:class:`~oci.limits.models.Quota` acted upon
        to enter the given state(s).
        ...
        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.limits.QuotasClient.remove_quota_lock`
