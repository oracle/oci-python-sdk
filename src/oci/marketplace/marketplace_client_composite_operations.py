...
class MarketplaceClientCompositeOperations(object):
    ...
    def create_publication_and_wait_for_state(self, create_publication_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        ...
        publication_id = operation_result.data.id

        try:
            waiter_result = oci.wait_until(
                self.client,
                self.client.get_publication(publication_id),  # noqa: F821
    ...
    def delete_publication_and_wait_for_state(self, publication_id, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        ...
        try:
            if ("succeed_on_not_found" in waiter_kwargs) and (waiter_kwargs["succeed_on_not_found"] is False):
                self.client.base_client.logger.warning("The waiter kwarg succeed_on_not_found was passed as False for the delete composite operation delete_publication, this would result in the operation to fail if the resource is not found! Please, do not pass this kwarg if this was not intended")
            else:
                """
