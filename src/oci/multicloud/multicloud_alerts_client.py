...
class MulticloudAlertsClient(object):
    """
    ...
    def __init__(self, config, **kwargs):
        ...
        self.base_client = BaseClient("multicloud_alerts", config, signer, multicloud_type_mapping, **base_client_init_kwargs)
        self.retry_strategy = kwargs.get('retry_strategy')
        self.circuit_breaker_callback = kwargs.get('circuit_breaker_callback')

    def list_multicloud_alerts(self, compartment_id, **kwargs):
        """
        ...
        :param str alert_type: (optional)
            Filter alerts by alert type (e.g. IAM_POLICY_GAP).

        :param str alert_function_name: (optional)
        ...
        :return: A :class:`~oci.response.Response` object with data of type :class:`~oci.multicloud.models.MulticloudAlertCollection`
        :rtype: :class:`~oci.response.Response`

        :example:
        Click `here <https://docs.oracle.com/en-us/iaas/tools/python-sdk-examples/latest/multicloud/list_multicloud_alerts.py.html>`__ to see an example of how to use list_multicloud_alerts API.
        """
        # Required path and query arguments. These are in camelCase to replace values in service endpoints.
        required_arguments = ['compartmentId']
        resource_path = "/multicloudalerts"
        method = "GET"
        operation_name = "list_multicloud_alerts"
        api_reference_link = "https://docs.oracle.com/iaas/api/#/en/multicloud-omhub-cp/20180828/MulticloudAlertCollection/ListMulticloudAlerts"
        ...
        if extra_kwargs:
            raise ValueError(
                f"list_multicloud_alerts got unknown kwargs: {extra_kwargs!r}")

        if 'severity' in kwargs:
            severity_allowed_values = ["LOW", "MEDIUM", "HIGH", "CRITICAL"]
        ...
        if 'alert_status' in kwargs:
            alert_status_allowed_values = ["UNACKNOWLEDGED", "ACKNOWLEDGED", "RESOLVED"]
            if kwargs['alert_status'] not in alert_status_allowed_values:
                raise ValueError(
                    f"Invalid value for `alert_status`, must be one of { alert_status_allowed_values }"
                )
        ...
        query_params = {
            ...
            "alertType": kwargs.get("alert_type", missing),
            "alertFunctionName": kwargs.get("alert_function_name", missing),
            "severity": kwargs.get("severity", missing),
            "alertStatus": kwargs.get("alert_status", missing),
        ...
        if retry_strategy:
            ...
            return retry_strategy.make_retrying_call(
                ...
                response_type="MulticloudAlertCollection",
