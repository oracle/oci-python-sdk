...
class RoverEntitlementClientCompositeOperations(object):
    ...
    def create_rover_entitlement_and_wait_for_state(self, create_rover_entitlement_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        ...
        rover_entitlement_id = operation_result.data.id

        try:
            waiter_result = oci.wait_until(
                self.client,
                self.client.get_rover_entitlement(rover_entitlement_id, compartment_id),  # noqa: F821
    ...
    def delete_rover_entitlement_and_wait_for_state(self, rover_entitlement_id, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        ...
        try:
            if ("succeed_on_not_found" in waiter_kwargs) and (waiter_kwargs["succeed_on_not_found"] is False):
                self.client.base_client.logger.warning("The waiter kwarg succeed_on_not_found was passed as False for the delete composite operation delete_rover_entitlement, this would result in the operation to fail if the resource is not found! Please, do not pass this kwarg if this was not intended")
            else:
                """
