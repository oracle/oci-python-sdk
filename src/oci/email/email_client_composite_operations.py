...
class EmailClientCompositeOperations(object):
    ...
    def add_email_domain_lock_and_wait_for_state(self, email_domain_id, add_lock_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.email.EmailClient.add_email_domain_lock` and waits for the :py:class:`~oci.email.models.EmailDomain` acted upon
        ...
        :param list[str] wait_for_states:
            An array of states to wait on. These should be valid values for :py:attr:`~oci.email.models.EmailDomain.lifecycle_state`

        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.email.EmailClient.add_email_domain_lock`

        :param dict waiter_kwargs:
            A dictionary of keyword arguments to pass to the :py:func:`oci.wait_until` function. For example, you could pass ``max_interval_seconds`` or ``max_interval_seconds``
            as dictionary keys to modify how long the waiter function will wait between retries and the maximum amount of time it will wait
        """
        operation_result = self.client.add_email_domain_lock(email_domain_id, add_lock_details, **operation_kwargs)
        if not wait_for_states:
            return operation_result
        lowered_wait_for_states = [w.lower() for w in wait_for_states]
        email_domain_id = operation_result.data.id

        try:
            waiter_result = oci.wait_until(
                self.client,
                self.client.get_email_domain(email_domain_id),  # noqa: F821
    ...
    def add_email_ip_pool_lock_and_wait_for_state(self, email_ip_pool_id, add_lock_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.email.EmailClient.add_email_ip_pool_lock` and waits for the :py:class:`~oci.email.models.EmailIpPool` acted upon
        to enter the given state(s).
        ...
        :param list[str] wait_for_states:
            An array of states to wait on. These should be valid values for :py:attr:`~oci.email.models.EmailIpPool.lifecycle_state`

        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.email.EmailClient.add_email_ip_pool_lock`

        :param dict waiter_kwargs:
            A dictionary of keyword arguments to pass to the :py:func:`oci.wait_until` function. For example, you could pass ``max_interval_seconds`` or ``max_interval_seconds``
            as dictionary keys to modify how long the waiter function will wait between retries and the maximum amount of time it will wait
        """
        operation_result = self.client.add_email_ip_pool_lock(email_ip_pool_id, add_lock_details, **operation_kwargs)
        if not wait_for_states:
            return operation_result
        lowered_wait_for_states = [w.lower() for w in wait_for_states]
        email_ip_pool_id = operation_result.data.id

        try:
            waiter_result = oci.wait_until(
                self.client,
                self.client.get_email_ip_pool(email_ip_pool_id),  # noqa: F821
                evaluate_response=lambda r: getattr(r.data, 'lifecycle_state') and getattr(r.data, 'lifecycle_state').lower() in lowered_wait_for_states,
                **waiter_kwargs
            )
            result_to_return = waiter_result
    ...
    def add_email_outbound_ip_and_wait_for_state(self, add_email_outbound_ip_details, email_ip_pool_id, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.email.EmailClient.add_email_outbound_ip` and waits for the :py:class:`~oci.email.models.WorkRequest`
        to enter the given state(s).
        ...
        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.email.EmailClient.add_email_outbound_ip`

        :param dict waiter_kwargs:
            A dictionary of keyword arguments to pass to the :py:func:`oci.wait_until` function. For example, you could pass ``max_interval_seconds`` or ``max_interval_seconds``
            as dictionary keys to modify how long the waiter function will wait between retries and the maximum amount of time it will wait
        """
        operation_result = self.client.add_email_outbound_ip(add_email_outbound_ip_details, email_ip_pool_id, **operation_kwargs)
        if not wait_for_states:
            return operation_result
        lowered_wait_for_states = [w.lower() for w in wait_for_states]
    ...
    def add_return_path_lock_and_wait_for_state(self, email_return_path_id, add_lock_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.email.EmailClient.add_return_path_lock` and waits for the :py:class:`~oci.email.models.EmailReturnPath` acted upon
        to enter the given state(s).
        ...
        :param list[str] wait_for_states:
            An array of states to wait on. These should be valid values for :py:attr:`~oci.email.models.EmailReturnPath.lifecycle_state`

        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.email.EmailClient.add_return_path_lock`

        :param dict waiter_kwargs:
            A dictionary of keyword arguments to pass to the :py:func:`oci.wait_until` function. For example, you could pass ``max_interval_seconds`` or ``max_interval_seconds``
            as dictionary keys to modify how long the waiter function will wait between retries and the maximum amount of time it will wait
        """
        operation_result = self.client.add_return_path_lock(email_return_path_id, add_lock_details, **operation_kwargs)
        if not wait_for_states:
            return operation_result
        lowered_wait_for_states = [w.lower() for w in wait_for_states]
        email_return_path_id = operation_result.data.id

        try:
            waiter_result = oci.wait_until(
                self.client,
                self.client.get_email_return_path(email_return_path_id),  # noqa: F821
                evaluate_response=lambda r: getattr(r.data, 'lifecycle_state') and getattr(r.data, 'lifecycle_state').lower() in lowered_wait_for_states,
                **waiter_kwargs
            )
            result_to_return = waiter_result
    ...
    def add_sender_lock_and_wait_for_state(self, sender_id, add_lock_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.email.EmailClient.add_sender_lock` and waits for the :py:class:`~oci.email.models.Sender` acted upon
        to enter the given state(s).
        ...
        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.email.EmailClient.add_sender_lock`

        :param dict waiter_kwargs:
            A dictionary of keyword arguments to pass to the :py:func:`oci.wait_until` function. For example, you could pass ``max_interval_seconds`` or ``max_interval_seconds``
            as dictionary keys to modify how long the waiter function will wait between retries and the maximum amount of time it will wait
        """
        operation_result = self.client.add_sender_lock(sender_id, add_lock_details, **operation_kwargs)
        if not wait_for_states:
            return operation_result
        lowered_wait_for_states = [w.lower() for w in wait_for_states]
    ...
    def change_email_ip_pool_compartment_and_wait_for_state(self, email_ip_pool_id, change_email_ip_pool_compartment_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.email.EmailClient.change_email_ip_pool_compartment` and waits for the :py:class:`~oci.email.models.WorkRequest`
        to enter the given state(s).
        ...
        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.email.EmailClient.change_email_ip_pool_compartment`

        :param dict waiter_kwargs:
            A dictionary of keyword arguments to pass to the :py:func:`oci.wait_until` function. For example, you could pass ``max_interval_seconds`` or ``max_interval_seconds``
            as dictionary keys to modify how long the waiter function will wait between retries and the maximum amount of time it will wait
        """
        operation_result = self.client.change_email_ip_pool_compartment(email_ip_pool_id, change_email_ip_pool_compartment_details, **operation_kwargs)
        if not wait_for_states:
            return operation_result
        lowered_wait_for_states = [w.lower() for w in wait_for_states]
        if 'opc-work-request-id' not in operation_result.headers:
    ...
    def create_email_ip_pool_and_wait_for_state(self, create_email_ip_pool_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.email.EmailClient.create_email_ip_pool` and waits for the :py:class:`~oci.email.models.WorkRequest`
        to enter the given state(s).
        ...
        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.email.EmailClient.create_email_ip_pool`

        :param dict waiter_kwargs:
            A dictionary of keyword arguments to pass to the :py:func:`oci.wait_until` function. For example, you could pass ``max_interval_seconds`` or ``max_interval_seconds``
            as dictionary keys to modify how long the waiter function will wait between retries and the maximum amount of time it will wait
        """
        operation_result = self.client.create_email_ip_pool(create_email_ip_pool_details, **operation_kwargs)
        if not wait_for_states:
            return operation_result
        lowered_wait_for_states = [w.lower() for w in wait_for_states]
        if 'opc-work-request-id' not in operation_result.headers:
    ...
    def delete_email_ip_pool_and_wait_for_state(self, email_ip_pool_id, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.email.EmailClient.delete_email_ip_pool` and waits for the :py:class:`~oci.email.models.WorkRequest`
        to enter the given state(s).
        ...
        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.email.EmailClient.delete_email_ip_pool`
        ...
        operation_result = None
        try:
            operation_result = self.client.delete_email_ip_pool(email_ip_pool_id, **operation_kwargs)
        except oci.exceptions.ServiceError as e:
            if e.status == 404:
                return WAIT_RESOURCE_NOT_FOUND
            else:
    ...
    def remove_email_domain_lock_and_wait_for_state(self, email_domain_id, remove_lock_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.email.EmailClient.remove_email_domain_lock` and waits for the :py:class:`~oci.email.models.EmailDomain` acted upon
        to enter the given state(s).
        ...
        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.email.EmailClient.remove_email_domain_lock`

        :param dict waiter_kwargs:
            A dictionary of keyword arguments to pass to the :py:func:`oci.wait_until` function. For example, you could pass ``max_interval_seconds`` or ``max_interval_seconds``
            as dictionary keys to modify how long the waiter function will wait between retries and the maximum amount of time it will wait
        """
        operation_result = self.client.remove_email_domain_lock(email_domain_id, remove_lock_details, **operation_kwargs)
        if not wait_for_states:
            return operation_result
        lowered_wait_for_states = [w.lower() for w in wait_for_states]
        email_domain_id = operation_result.data.id
    ...
    def remove_email_ip_pool_lock_and_wait_for_state(self, email_ip_pool_id, remove_lock_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.email.EmailClient.remove_email_ip_pool_lock` and waits for the :py:class:`~oci.email.models.EmailIpPool` acted upon
        to enter the given state(s).
        ...
        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.email.EmailClient.remove_email_ip_pool_lock`

        :param dict waiter_kwargs:
            A dictionary of keyword arguments to pass to the :py:func:`oci.wait_until` function. For example, you could pass ``max_interval_seconds`` or ``max_interval_seconds``
            as dictionary keys to modify how long the waiter function will wait between retries and the maximum amount of time it will wait
        """
        operation_result = self.client.remove_email_ip_pool_lock(email_ip_pool_id, remove_lock_details, **operation_kwargs)
        if not wait_for_states:
            return operation_result
        lowered_wait_for_states = [w.lower() for w in wait_for_states]
        email_ip_pool_id = operation_result.data.id
    ...
    def remove_email_outbound_ip_and_wait_for_state(self, remove_email_outbound_ip_details, email_ip_pool_id, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.email.EmailClient.remove_email_outbound_ip` and waits for the :py:class:`~oci.email.models.WorkRequest`
        to enter the given state(s).
        ...
        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.email.EmailClient.remove_email_outbound_ip`

        :param dict waiter_kwargs:
            A dictionary of keyword arguments to pass to the :py:func:`oci.wait_until` function. For example, you could pass ``max_interval_seconds`` or ``max_interval_seconds``
            as dictionary keys to modify how long the waiter function will wait between retries and the maximum amount of time it will wait
        """
        operation_result = self.client.remove_email_outbound_ip(remove_email_outbound_ip_details, email_ip_pool_id, **operation_kwargs)
        if not wait_for_states:
            return operation_result
        lowered_wait_for_states = [w.lower() for w in wait_for_states]
        if 'opc-work-request-id' not in operation_result.headers:
    ...
    def remove_return_path_lock_and_wait_for_state(self, email_return_path_id, remove_lock_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.email.EmailClient.remove_return_path_lock` and waits for the :py:class:`~oci.email.models.EmailReturnPath` acted upon
        to enter the given state(s).
        ...
        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.email.EmailClient.remove_return_path_lock`

        :param dict waiter_kwargs:
            A dictionary of keyword arguments to pass to the :py:func:`oci.wait_until` function. For example, you could pass ``max_interval_seconds`` or ``max_interval_seconds``
            as dictionary keys to modify how long the waiter function will wait between retries and the maximum amount of time it will wait
        """
        operation_result = self.client.remove_return_path_lock(email_return_path_id, remove_lock_details, **operation_kwargs)
        if not wait_for_states:
            return operation_result
        lowered_wait_for_states = [w.lower() for w in wait_for_states]
        email_return_path_id = operation_result.data.id
    ...
    def remove_sender_lock_and_wait_for_state(self, sender_id, remove_lock_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.email.EmailClient.remove_sender_lock` and waits for the :py:class:`~oci.email.models.Sender` acted upon
        to enter the given state(s).
        ...
        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.email.EmailClient.remove_sender_lock`

        :param dict waiter_kwargs:
            A dictionary of keyword arguments to pass to the :py:func:`oci.wait_until` function. For example, you could pass ``max_interval_seconds`` or ``max_interval_seconds``
            as dictionary keys to modify how long the waiter function will wait between retries and the maximum amount of time it will wait
        """
        operation_result = self.client.remove_sender_lock(sender_id, remove_lock_details, **operation_kwargs)
        if not wait_for_states:
            return operation_result
        lowered_wait_for_states = [w.lower() for w in wait_for_states]
        sender_id = operation_result.data.id
    ...
    def update_email_ip_pool_and_wait_for_state(self, email_ip_pool_id, update_email_ip_pool_details, wait_for_states=[], operation_kwargs={}, waiter_kwargs={}):
        """
        Calls :py:func:`~oci.email.EmailClient.update_email_ip_pool` and waits for the :py:class:`~oci.email.models.WorkRequest`
        to enter the given state(s).
        ...
        :param dict operation_kwargs:
            A dictionary of keyword arguments to pass to :py:func:`~oci.email.EmailClient.update_email_ip_pool`

        :param dict waiter_kwargs:
            A dictionary of keyword arguments to pass to the :py:func:`oci.wait_until` function. For example, you could pass ``max_interval_seconds`` or ``max_interval_seconds``
            as dictionary keys to modify how long the waiter function will wait between retries and the maximum amount of time it will wait
        """
        operation_result = self.client.update_email_ip_pool(email_ip_pool_id, update_email_ip_pool_details, **operation_kwargs)
        if not wait_for_states:
            return operation_result
        lowered_wait_for_states = [w.lower() for w in wait_for_states]
        if 'opc-work-request-id' not in operation_result.headers:
