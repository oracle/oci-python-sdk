...
def test_create_user(testing_service_client):
    if not testing_service_client.is_api_enabled('cims', 'CreateUser'):
        pytest.skip('OCI Testing Service has not been configured for this operation yet.')

    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('cims', util.camelize('user'), 'CreateUser')
    )

    request_containers = testing_service_client.get_requests(service_name='cims', api_name='CreateUser')
    ...
    for i in range(len(request_containers)):
        ...
        try:
            ...
            client = oci.cims.UserClient(config, service_endpoint=service_endpoint)
