...
def test_validate_connection(testing_service_client):
    if not testing_service_client.is_api_enabled('devops', 'ValidateConnection'):
        pytest.skip('OCI Testing Service has not been configured for this operation yet.')

    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('devops', util.camelize('devops'), 'ValidateConnection')
    )

    request_containers = testing_service_client.get_requests(service_name='devops', api_name='ValidateConnection')
