...
def test_invoke_function(testing_service_client):
    if not testing_service_client.is_api_enabled('functions', 'InvokeFunction'):
        pytest.skip('OCI Testing Service has not been configured for this operation yet.')

    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('functions', util.camelize('functions_invoke'), 'InvokeFunction')
    )

    request_containers = testing_service_client.get_requests(service_name='functions', api_name='InvokeFunction')
