...
# IssueRoutingInfo tag="c3" email="c3_scrum_team_us_grp@oracle.com" jiraProject="RSC" opsJiraProject="RSC"
def test_list_virtual_circuit_associated_tunnels(testing_service_client):
    if not testing_service_client.is_api_enabled('core', 'ListVirtualCircuitAssociatedTunnels'):
        pytest.skip('OCI Testing Service has not been configured for this operation yet.')

    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('core', util.camelize('virtual_network'), 'ListVirtualCircuitAssociatedTunnels')
    )
    mock_mode = config['test_mode'] == 'mock' if 'test_mode' in config else False

    request_containers = testing_service_client.get_requests(service_name='core', api_name='ListVirtualCircuitAssociatedTunnels')
    ...
    for i in range(len(request_containers)):
        ...
        try:
            service_endpoint = config['endpoint'] if 'endpoint' in config else None
            client = oci.core.VirtualNetworkClient(config, service_endpoint=service_endpoint)
            response = client.list_virtual_circuit_associated_tunnels(
                virtual_circuit_id=request.pop(util.camelize('virtualCircuitId')),
                retry_strategy=oci.retry.NoneRetryStrategy(),
                **(util.camel_to_snake_keys(request))
            )
            ...
            if not mock_mode and response.has_next_page:
                next_page = response.headers['opc-next-page']
                request = request_containers[i]['request'].copy()
                next_response = client.list_virtual_circuit_associated_tunnels(
                    virtual_circuit_id=request.pop(util.camelize('virtualCircuitId')),
                    page=next_page,
                    retry_strategy=oci.retry.NoneRetryStrategy(),
                    **(util.camel_to_snake_keys(request))
                ...
                if prev_page in next_response.headers:
                    request = request_containers[i]['request'].copy()
                    prev_response = client.list_virtual_circuit_associated_tunnels(
                        virtual_circuit_id=request.pop(util.camelize('virtualCircuitId')),
                        page=next_response.headers[prev_page],
                        retry_strategy=oci.retry.NoneRetryStrategy(),
                        **(util.camel_to_snake_keys(request))
