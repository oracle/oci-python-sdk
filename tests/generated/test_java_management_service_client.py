...
# IssueRoutingInfo tag="default" email="autonomous_java-dev_us_grp@oracle.com" jiraProject="AJ" opsJiraProject="AJ"
def test_request_summarized_application_usage(testing_service_client):
    if not testing_service_client.is_api_enabled('jms', 'RequestSummarizedApplicationUsage'):
        pytest.skip('OCI Testing Service has not been configured for this operation yet.')

    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('jms', util.camelize('java_management_service'), 'RequestSummarizedApplicationUsage')
    )
    mock_mode = config['test_mode'] == 'mock' if 'test_mode' in config else False

    request_containers = testing_service_client.get_requests(service_name='jms', api_name='RequestSummarizedApplicationUsage')
    ...
    for i in range(len(request_containers)):
        ...
        try:
            service_endpoint = config['endpoint'] if 'endpoint' in config else None
            client = oci.jms.JavaManagementServiceClient(config, service_endpoint=service_endpoint)
            response = client.request_summarized_application_usage(
                fleet_id=request.pop(util.camelize('fleetId')),
                request_summarized_application_usage_details=request.pop(util.camelize('RequestSummarizedApplicationUsageDetails')),
                **(util.camel_to_snake_keys(request))
            )
            result.append(response)
            if not mock_mode and response.has_next_page:
                next_page = response.headers['opc-next-page']
                request = request_containers[i]['request'].copy()
                next_response = client.request_summarized_application_usage(
                    fleet_id=request.pop(util.camelize('fleetId')),
                    request_summarized_application_usage_details=request.pop(util.camelize('RequestSummarizedApplicationUsageDetails')),
                    page=next_page,
                    **(util.camel_to_snake_keys(request))
                ...
                if prev_page in next_response.headers:
                    request = request_containers[i]['request'].copy()
                    prev_response = client.request_summarized_application_usage(
                        fleet_id=request.pop(util.camelize('fleetId')),
                        request_summarized_application_usage_details=request.pop(util.camelize('RequestSummarizedApplicationUsageDetails')),
                        page=next_response.headers[prev_page],
                        **(util.camel_to_snake_keys(request))
...
# IssueRoutingInfo tag="default" email="autonomous_java-dev_us_grp@oracle.com" jiraProject="AJ" opsJiraProject="AJ"
def test_request_summarized_installation_usage(testing_service_client):
    if not testing_service_client.is_api_enabled('jms', 'RequestSummarizedInstallationUsage'):
        pytest.skip('OCI Testing Service has not been configured for this operation yet.')

    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('jms', util.camelize('java_management_service'), 'RequestSummarizedInstallationUsage')
    )
    mock_mode = config['test_mode'] == 'mock' if 'test_mode' in config else False

    request_containers = testing_service_client.get_requests(service_name='jms', api_name='RequestSummarizedInstallationUsage')
    ...
    for i in range(len(request_containers)):
        ...
        try:
            service_endpoint = config['endpoint'] if 'endpoint' in config else None
            client = oci.jms.JavaManagementServiceClient(config, service_endpoint=service_endpoint)
            response = client.request_summarized_installation_usage(
                fleet_id=request.pop(util.camelize('fleetId')),
                request_summarized_installation_usage_details=request.pop(util.camelize('RequestSummarizedInstallationUsageDetails')),
                **(util.camel_to_snake_keys(request))
            )
            result.append(response)
            if not mock_mode and response.has_next_page:
                next_page = response.headers['opc-next-page']
                request = request_containers[i]['request'].copy()
                next_response = client.request_summarized_installation_usage(
                    fleet_id=request.pop(util.camelize('fleetId')),
                    request_summarized_installation_usage_details=request.pop(util.camelize('RequestSummarizedInstallationUsageDetails')),
                    page=next_page,
                    **(util.camel_to_snake_keys(request))
                ...
                if prev_page in next_response.headers:
                    request = request_containers[i]['request'].copy()
                    prev_response = client.request_summarized_installation_usage(
                        fleet_id=request.pop(util.camelize('fleetId')),
                        request_summarized_installation_usage_details=request.pop(util.camelize('RequestSummarizedInstallationUsageDetails')),
                        page=next_response.headers[prev_page],
                        **(util.camel_to_snake_keys(request))
...
# IssueRoutingInfo tag="default" email="autonomous_java-dev_us_grp@oracle.com" jiraProject="AJ" opsJiraProject="AJ"
def test_request_summarized_jre_usage(testing_service_client):
    if not testing_service_client.is_api_enabled('jms', 'RequestSummarizedJreUsage'):
        pytest.skip('OCI Testing Service has not been configured for this operation yet.')

    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('jms', util.camelize('java_management_service'), 'RequestSummarizedJreUsage')
    )
    mock_mode = config['test_mode'] == 'mock' if 'test_mode' in config else False

    request_containers = testing_service_client.get_requests(service_name='jms', api_name='RequestSummarizedJreUsage')
    ...
    for i in range(len(request_containers)):
        ...
        try:
            service_endpoint = config['endpoint'] if 'endpoint' in config else None
            client = oci.jms.JavaManagementServiceClient(config, service_endpoint=service_endpoint)
            response = client.request_summarized_jre_usage(
                fleet_id=request.pop(util.camelize('fleetId')),
                request_summarized_jre_usage_details=request.pop(util.camelize('RequestSummarizedJreUsageDetails')),
                **(util.camel_to_snake_keys(request))
            )
            result.append(response)
            if not mock_mode and response.has_next_page:
                next_page = response.headers['opc-next-page']
                request = request_containers[i]['request'].copy()
                next_response = client.request_summarized_jre_usage(
                    fleet_id=request.pop(util.camelize('fleetId')),
                    request_summarized_jre_usage_details=request.pop(util.camelize('RequestSummarizedJreUsageDetails')),
                    page=next_page,
                    **(util.camel_to_snake_keys(request))
                ...
                if prev_page in next_response.headers:
                    request = request_containers[i]['request'].copy()
                    prev_response = client.request_summarized_jre_usage(
                        fleet_id=request.pop(util.camelize('fleetId')),
                        request_summarized_jre_usage_details=request.pop(util.camelize('RequestSummarizedJreUsageDetails')),
                        page=next_response.headers[prev_page],
                        **(util.camel_to_snake_keys(request))
...
# IssueRoutingInfo tag="default" email="autonomous_java-dev_us_grp@oracle.com" jiraProject="AJ" opsJiraProject="AJ"
def test_request_summarized_managed_instance_usage(testing_service_client):
    if not testing_service_client.is_api_enabled('jms', 'RequestSummarizedManagedInstanceUsage'):
        pytest.skip('OCI Testing Service has not been configured for this operation yet.')

    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('jms', util.camelize('java_management_service'), 'RequestSummarizedManagedInstanceUsage')
    )
    mock_mode = config['test_mode'] == 'mock' if 'test_mode' in config else False

    request_containers = testing_service_client.get_requests(service_name='jms', api_name='RequestSummarizedManagedInstanceUsage')
    ...
    for i in range(len(request_containers)):
        ...
        try:
            service_endpoint = config['endpoint'] if 'endpoint' in config else None
            client = oci.jms.JavaManagementServiceClient(config, service_endpoint=service_endpoint)
            response = client.request_summarized_managed_instance_usage(
                fleet_id=request.pop(util.camelize('fleetId')),
                request_summarized_managed_instance_usage_details=request.pop(util.camelize('RequestSummarizedManagedInstanceUsageDetails')),
                **(util.camel_to_snake_keys(request))
            )
            result.append(response)
            if not mock_mode and response.has_next_page:
                next_page = response.headers['opc-next-page']
                request = request_containers[i]['request'].copy()
                next_response = client.request_summarized_managed_instance_usage(
                    fleet_id=request.pop(util.camelize('fleetId')),
                    request_summarized_managed_instance_usage_details=request.pop(util.camelize('RequestSummarizedManagedInstanceUsageDetails')),
                    page=next_page,
                    **(util.camel_to_snake_keys(request))
                ...
                if prev_page in next_response.headers:
                    request = request_containers[i]['request'].copy()
                    prev_response = client.request_summarized_managed_instance_usage(
                        fleet_id=request.pop(util.camelize('fleetId')),
                        request_summarized_managed_instance_usage_details=request.pop(util.camelize('RequestSummarizedManagedInstanceUsageDetails')),
                        page=next_response.headers[prev_page],
                        **(util.camel_to_snake_keys(request))
