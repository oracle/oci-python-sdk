...
def test_list_shapes(testing_service_client):
    if not testing_service_client.is_api_enabled('rover', 'ListShapes'):
        pytest.skip('OCI Testing Service has not been configured for this operation yet.')

    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('rover', util.camelize('shape'), 'ListShapes')
    )
    mock_mode = config['test_mode'] == 'mock' if 'test_mode' in config else False

    request_containers = testing_service_client.get_requests(service_name='rover', api_name='ListShapes')
    ...
    for i in range(len(request_containers)):
        ...
        try:
            ...
            client = oci.rover.ShapeClient(config, service_endpoint=service_endpoint)
