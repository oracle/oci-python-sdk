...
# IssueRoutingInfo tag="default" email="team_oci_orm_us_grp@oracle.com" jiraProject="ORCH" opsJiraProject="RMS"
def test_list_job_associated_resources(testing_service_client):
    if not testing_service_client.is_api_enabled('resource_manager', 'ListJobAssociatedResources'):
        pytest.skip('OCI Testing Service has not been configured for this operation yet.')

    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('resource_manager', util.camelize('resource_manager'), 'ListJobAssociatedResources')
    )
    mock_mode = config['test_mode'] == 'mock' if 'test_mode' in config else False

    request_containers = testing_service_client.get_requests(service_name='resource_manager', api_name='ListJobAssociatedResources')
    ...
    for i in range(len(request_containers)):
        ...
        try:
            service_endpoint = config['endpoint'] if 'endpoint' in config else None
            client = oci.resource_manager.ResourceManagerClient(config, service_endpoint=service_endpoint)
            response = client.list_job_associated_resources(
                job_id=request.pop(util.camelize('jobId')),
                retry_strategy=oci.retry.NoneRetryStrategy(),
                **(util.camel_to_snake_keys(request))
            )
            ...
            if not mock_mode and response.has_next_page:
                next_page = response.headers['opc-next-page']
                request = request_containers[i]['request'].copy()
                next_response = client.list_job_associated_resources(
                    job_id=request.pop(util.camelize('jobId')),
                    page=next_page,
                    retry_strategy=oci.retry.NoneRetryStrategy(),
                    **(util.camel_to_snake_keys(request))
                ...
                if prev_page in next_response.headers:
                    request = request_containers[i]['request'].copy()
                    prev_response = client.list_job_associated_resources(
                        job_id=request.pop(util.camelize('jobId')),
                        page=next_response.headers[prev_page],
                        retry_strategy=oci.retry.NoneRetryStrategy(),
                        **(util.camel_to_snake_keys(request))
...
def test_list_job_outputs(testing_service_client):
    if not testing_service_client.is_api_enabled('resource_manager', 'ListJobOutputs'):
        pytest.skip('OCI Testing Service has not been configured for this operation yet.')

    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('resource_manager', util.camelize('resource_manager'), 'ListJobOutputs')
    )
    mock_mode = config['test_mode'] == 'mock' if 'test_mode' in config else False

    request_containers = testing_service_client.get_requests(service_name='resource_manager', api_name='ListJobOutputs')
...
# IssueRoutingInfo tag="default" email="team_oci_orm_us_grp@oracle.com" jiraProject="ORCH" opsJiraProject="RMS"
def test_list_stack_associated_resources(testing_service_client):
    if not testing_service_client.is_api_enabled('resource_manager', 'ListStackAssociatedResources'):
        pytest.skip('OCI Testing Service has not been configured for this operation yet.')

    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('resource_manager', util.camelize('resource_manager'), 'ListStackAssociatedResources')
    )
    mock_mode = config['test_mode'] == 'mock' if 'test_mode' in config else False

    request_containers = testing_service_client.get_requests(service_name='resource_manager', api_name='ListStackAssociatedResources')
    ...
    for i in range(len(request_containers)):
        ...
        try:
            service_endpoint = config['endpoint'] if 'endpoint' in config else None
            client = oci.resource_manager.ResourceManagerClient(config, service_endpoint=service_endpoint)
            response = client.list_stack_associated_resources(
                stack_id=request.pop(util.camelize('stackId')),
                retry_strategy=oci.retry.NoneRetryStrategy(),
                **(util.camel_to_snake_keys(request))
            )
            ...
            if not mock_mode and response.has_next_page:
                next_page = response.headers['opc-next-page']
                request = request_containers[i]['request'].copy()
                next_response = client.list_stack_associated_resources(
                    stack_id=request.pop(util.camelize('stackId')),
                    page=next_page,
                    retry_strategy=oci.retry.NoneRetryStrategy(),
                    **(util.camel_to_snake_keys(request))
                ...
                if prev_page in next_response.headers:
                    request = request_containers[i]['request'].copy()
                    prev_response = client.list_stack_associated_resources(
                        stack_id=request.pop(util.camelize('stackId')),
                        page=next_response.headers[prev_page],
                        retry_strategy=oci.retry.NoneRetryStrategy(),
                        **(util.camel_to_snake_keys(request))
