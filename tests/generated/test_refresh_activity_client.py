...
def test_create_refresh_activity(testing_service_client):
    ...
    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('fusion_apps', util.camelize('refresh_activity'), 'CreateRefreshActivity')
    )
    ...
    for i in range(len(request_containers)):
        ...
        try:
            ...
            client = oci.fusion_apps.RefreshActivityClient(config, service_endpoint=service_endpoint)
...
def test_get_refresh_activity(testing_service_client):
    ...
    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('fusion_apps', util.camelize('refresh_activity'), 'GetRefreshActivity')
    )
...
def test_list_refresh_activities(testing_service_client):
    ...
    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('fusion_apps', util.camelize('refresh_activity'), 'ListRefreshActivities')
    )
...
def test_list_time_available_for_refreshes(testing_service_client):
    ...
    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('fusion_apps', util.camelize('refresh_activity'), 'ListTimeAvailableForRefreshes')
    )
    mock_mode = config['test_mode'] == 'mock' if 'test_mode' in config else False
