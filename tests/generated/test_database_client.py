...
def test_cancel_backup(testing_service_client):
    ...
    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('database', util.camelize('database'), 'CancelBackup')
    )

    request_containers = testing_service_client.get_requests(service_name='database', api_name='CancelBackup')
...
def test_change_dataguard_role(testing_service_client):
    ...
    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('database', util.camelize('database'), 'ChangeDataguardRole')
    )

    request_containers = testing_service_client.get_requests(service_name='database', api_name='ChangeDataguardRole')
    ...
    for i in range(len(request_containers)):
        ...
        try:
            service_endpoint = config['endpoint'] if 'endpoint' in config else None
            client = oci.database.DatabaseClient(config, service_endpoint=service_endpoint)
            response = client.change_dataguard_role(
                change_dataguard_role_details=request.pop(util.camelize('ChangeDataguardRoleDetails')),
                autonomous_container_database_id=request.pop(util.camelize('autonomousContainerDatabaseId')),
                retry_strategy=oci.retry.NoneRetryStrategy(),
                **(util.camel_to_snake_keys(request))
            )
...
def test_change_disaster_recovery_configuration(testing_service_client):
    ...
    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('database', util.camelize('database'), 'ChangeDisasterRecoveryConfiguration')
    )

    request_containers = testing_service_client.get_requests(service_name='database', api_name='ChangeDisasterRecoveryConfiguration')
    ...
    for i in range(len(request_containers)):
        ...
        try:
            service_endpoint = config['endpoint'] if 'endpoint' in config else None
            client = oci.database.DatabaseClient(config, service_endpoint=service_endpoint)
            response = client.change_disaster_recovery_configuration(
                autonomous_database_id=request.pop(util.camelize('autonomousDatabaseId')),
                change_disaster_recovery_configuration_details=request.pop(util.camelize('ChangeDisasterRecoveryConfigurationDetails')),
                retry_strategy=oci.retry.NoneRetryStrategy(),
                **(util.camel_to_snake_keys(request))
            )
            result.append(response)
...
# IssueRoutingInfo tag="dbaas-atp-d" email="sic_dbaas_cp_us_grp@oracle.com" jiraProject="DBAASADBD" opsJiraProject="DBAASADBD"
def test_get_autonomous_container_database_resource_usage(testing_service_client):
    if not testing_service_client.is_api_enabled('database', 'GetAutonomousContainerDatabaseResourceUsage'):
        pytest.skip('OCI Testing Service has not been configured for this operation yet.')

    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('database', util.camelize('database'), 'GetAutonomousContainerDatabaseResourceUsage')
    )

    request_containers = testing_service_client.get_requests(service_name='database', api_name='GetAutonomousContainerDatabaseResourceUsage')
    ...
    for i in range(len(request_containers)):
        ...
        try:
            service_endpoint = config['endpoint'] if 'endpoint' in config else None
            client = oci.database.DatabaseClient(config, service_endpoint=service_endpoint)
            response = client.get_autonomous_container_database_resource_usage(
                autonomous_container_database_id=request.pop(util.camelize('autonomousContainerDatabaseId')),
                retry_strategy=oci.retry.NoneRetryStrategy(),
                **(util.camel_to_snake_keys(request))
            )
...
def test_get_autonomous_virtual_machine(testing_service_client):
    ...
    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('database', util.camelize('database'), 'GetAutonomousVirtualMachine')
    )

    request_containers = testing_service_client.get_requests(service_name='database', api_name='GetAutonomousVirtualMachine')
    ...
    for i in range(len(request_containers)):
        ...
        try:
            service_endpoint = config['endpoint'] if 'endpoint' in config else None
            client = oci.database.DatabaseClient(config, service_endpoint=service_endpoint)
            response = client.get_autonomous_virtual_machine(
                autonomous_virtual_machine_id=request.pop(util.camelize('autonomousVirtualMachineId')),
                retry_strategy=oci.retry.NoneRetryStrategy(),
                **(util.camel_to_snake_keys(request))
            )
            result.append(response)
...
# IssueRoutingInfo tag="dbaas-atp-d" email="sic_dbaas_cp_us_grp@oracle.com" jiraProject="DBAASADBD" opsJiraProject="DBAASADBD"
def test_get_cloud_autonomous_vm_cluster_resource_usage(testing_service_client):
    if not testing_service_client.is_api_enabled('database', 'GetCloudAutonomousVmClusterResourceUsage'):
        pytest.skip('OCI Testing Service has not been configured for this operation yet.')

    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('database', util.camelize('database'), 'GetCloudAutonomousVmClusterResourceUsage')
    )

    request_containers = testing_service_client.get_requests(service_name='database', api_name='GetCloudAutonomousVmClusterResourceUsage')
    ...
    for i in range(len(request_containers)):
        ...
        try:
            service_endpoint = config['endpoint'] if 'endpoint' in config else None
            client = oci.database.DatabaseClient(config, service_endpoint=service_endpoint)
            response = client.get_cloud_autonomous_vm_cluster_resource_usage(
                cloud_autonomous_vm_cluster_id=request.pop(util.camelize('cloudAutonomousVmClusterId')),
                retry_strategy=oci.retry.NoneRetryStrategy(),
                **(util.camel_to_snake_keys(request))
            )
...
def test_list_autonomous_container_database_versions(testing_service_client):
    ...
    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('database', util.camelize('database'), 'ListAutonomousContainerDatabaseVersions')
    )
    mock_mode = config['test_mode'] == 'mock' if 'test_mode' in config else False

    request_containers = testing_service_client.get_requests(service_name='database', api_name='ListAutonomousContainerDatabaseVersions')
    ...
    for i in range(len(request_containers)):
        ...
        try:
            service_endpoint = config['endpoint'] if 'endpoint' in config else None
            client = oci.database.DatabaseClient(config, service_endpoint=service_endpoint)
            response = client.list_autonomous_container_database_versions(
                compartment_id=request.pop(util.camelize('compartmentId')),
                service_component=request.pop(util.camelize('serviceComponent')),
                retry_strategy=oci.retry.NoneRetryStrategy(),
                **(util.camel_to_snake_keys(request))
            )
            result.append(response)
            if not mock_mode and response.has_next_page:
                next_page = response.headers['opc-next-page']
                request = request_containers[i]['request'].copy()
                next_response = client.list_autonomous_container_database_versions(
                    compartment_id=request.pop(util.camelize('compartmentId')),
                    service_component=request.pop(util.camelize('serviceComponent')),
                    page=next_page,
                    retry_strategy=oci.retry.NoneRetryStrategy(),
                ...
                if prev_page in next_response.headers:
                    request = request_containers[i]['request'].copy()
                    prev_response = client.list_autonomous_container_database_versions(
                        compartment_id=request.pop(util.camelize('compartmentId')),
                        service_component=request.pop(util.camelize('serviceComponent')),
                        page=next_response.headers[prev_page],
                        retry_strategy=oci.retry.NoneRetryStrategy(),
...
def test_list_autonomous_virtual_machines(testing_service_client):
    ...
    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('database', util.camelize('database'), 'ListAutonomousVirtualMachines')
    )
    mock_mode = config['test_mode'] == 'mock' if 'test_mode' in config else False

    request_containers = testing_service_client.get_requests(service_name='database', api_name='ListAutonomousVirtualMachines')
    ...
    for i in range(len(request_containers)):
        ...
        try:
            ...
            if not mock_mode and response.has_next_page:
                next_page = response.headers['opc-next-page']
                request = request_containers[i]['request'].copy()
                next_response = client.list_autonomous_virtual_machines(
                    compartment_id=request.pop(util.camelize('compartmentId')),
                    autonomous_vm_cluster_id=request.pop(util.camelize('autonomousVmClusterId')),
                    page=next_page,
                    retry_strategy=oci.retry.NoneRetryStrategy(),
                ...
                if prev_page in next_response.headers:
                    request = request_containers[i]['request'].copy()
                    prev_response = client.list_autonomous_virtual_machines(
                        compartment_id=request.pop(util.camelize('compartmentId')),
                        autonomous_vm_cluster_id=request.pop(util.camelize('autonomousVmClusterId')),
                        page=next_response.headers[prev_page],
                        retry_strategy=oci.retry.NoneRetryStrategy(),
...
# IssueRoutingInfo tag="dbaas-atp-d" email="sic_dbaas_cp_us_grp@oracle.com" jiraProject="DBAASADBD" opsJiraProject="DBAASADBD"
def test_list_cloud_autonomous_vm_cluster_acd_resource_usage(testing_service_client):
    if not testing_service_client.is_api_enabled('database', 'ListCloudAutonomousVmClusterAcdResourceUsage'):
        pytest.skip('OCI Testing Service has not been configured for this operation yet.')

    config = util.test_config_to_python_config(
        testing_service_client.get_test_config('database', util.camelize('database'), 'ListCloudAutonomousVmClusterAcdResourceUsage')
    )
    mock_mode = config['test_mode'] == 'mock' if 'test_mode' in config else False

    request_containers = testing_service_client.get_requests(service_name='database', api_name='ListCloudAutonomousVmClusterAcdResourceUsage')
    ...
    for i in range(len(request_containers)):
        ...
        try:
            service_endpoint = config['endpoint'] if 'endpoint' in config else None
            client = oci.database.DatabaseClient(config, service_endpoint=service_endpoint)
            response = client.list_cloud_autonomous_vm_cluster_acd_resource_usage(
                cloud_autonomous_vm_cluster_id=request.pop(util.camelize('cloudAutonomousVmClusterId')),
                retry_strategy=oci.retry.NoneRetryStrategy(),
                **(util.camel_to_snake_keys(request))
            )
            ...
            if not mock_mode and response.has_next_page:
                next_page = response.headers['opc-next-page']
                request = request_containers[i]['request'].copy()
                next_response = client.list_cloud_autonomous_vm_cluster_acd_resource_usage(
                    cloud_autonomous_vm_cluster_id=request.pop(util.camelize('cloudAutonomousVmClusterId')),
                    page=next_page,
                    retry_strategy=oci.retry.NoneRetryStrategy(),
                    **(util.camel_to_snake_keys(request))
                ...
                if prev_page in next_response.headers:
                    request = request_containers[i]['request'].copy()
                    prev_response = client.list_cloud_autonomous_vm_cluster_acd_resource_usage(
                        cloud_autonomous_vm_cluster_id=request.pop(util.camelize('cloudAutonomousVmClusterId')),
                        page=next_response.headers[prev_page],
                        retry_strategy=oci.retry.NoneRetryStrategy(),
                        **(util.camel_to_snake_keys(request))
