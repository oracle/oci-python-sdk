...
class TestObjectStorage(ServiceTestBase):
    ...
    def test_object_CRUD(self):
        ...
        self.api.put_object(namespace, bucket_name, 'second_object', tests.util.random_number_string())
        self.api.put_object(namespace, bucket_name, 'third_object', tests.util.random_number_string())
    ...
    def test_bucket_not_found(self):
        ...
        with self.assertRaises(oraclebmc.exceptions.ServiceError) as errorContext:
            self.api.get_bucket(namespace, tests.util.unique_name('does_not_exist'))
    ...
    def test_list_buckets_truncated(self):
        ...
        response = self.api.list_buckets(namespace, self.config.tenancy, limit=2, page=response.next_page)
        self.assertEqual(200, response.status)
        self.assertEqual(2, len(response.data))
    ...
    def test_list_objects_with_prefix_and_delimiter(self):
        namespace = self.api.get_namespace().data
        response = self.api.list_objects(namespace, 'ReadOnlyTestBucket2', prefix='a/b/', delimiter='/')
        self.assertEqual(200, response.status)
        assert (type(response.data) is oraclebmc.models.ListObjects)
    ...
    def test_list_objects_truncated(self):
        ...
        response = self.api.list_objects(namespace, 'ReadOnlyTestBucket1', start=response.data.next_start_with)
        self.assertEqual(2, len(response.data.objects))
        self.assertIsNone(response.data.prefixes)
        self.assertIsNone(response.data.next_start_with)
