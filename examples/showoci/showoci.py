...
# @author      : Adi Zohar - adi.zohar@oracle.com
# @Version     : 3.0.3
#
...
def get_flags(cmd):
    flag=""
    if cmd.all       : flag+="all "
    if cmd.allnoiam  : flag+="all_no_iam "
    if cmd.compute   : flag+="compute "
    if cmd.database  : flag+="database "
    if cmd.email     : flag+="email "
    if cmd.file      : flag+="file "
    if cmd.identity  : flag+="identity "
    if cmd.load      : flag+="load "
    if cmd.kms       : flag+="kms "
    if cmd.network   : flag+="network "
    if cmd.object    : flag+="object "
    if cmd.proxy     : flag+="proxy "
    if cmd.compart   : flag+="compartment "
    if cmd.region    : flag+="region "
    if cmd.ocid      : flag+="ocid "
    if cmd.noroot    : flag+="noroot "
    if cmd.mgdcompart: flag+="MgdCompartment "
    if cmd.sumonly   : flag+="summary "
    return flag
...
def print_identity_providers(identity_providers):
    ...
    try:
        ...
        if not identity_providers: return
        ...
        for ip in identity_providers:
            ...
            for ig in ip['group_map']:  print (tabs + "Group Map : " + ig)
        print ("")
...
def print_identity_main(data):
    try:
        if 'tenancy' in data: print_identity_tenancy(data['tenancy'])
        if 'users'   in data: print_identity_users  (data['users'])
        if 'groups'  in data: print_identity_groups(data['groups'])
        if 'dynamic_groups' in data: print_identity_dynamic_groups(data['dynamic_groups'])
        if 'policies' in data: print_identity_policies(data['policies'])
        if 'providers' in data: print_identity_providers(data['providers'])
...
def print_core_network_vcn(vcns):
        ...
        for vcn in vcns:
            ...
            if 'data' in vcn:
                for data in vcn['data']:
                    if 'igw'          in data: 
                        if data['igw']!="" : print (tabs + "Internet GW : " + data['igw'] + print_core_network_vcn_compartment(vcn_compartment,data['compartment']))
                        
                for data in vcn['data']:
                    if 'sgw'          in data: 
                        if data['sgw']!="" : print (tabs + "Service GW  : " + data['sgw']+ print_core_network_vcn_compartment(vcn_compartment,data['compartment']))
                        
                for data in vcn['data']:
                    if 'nat'          in data: 
                        if data['nat']!="" : print (tabs + "NAT GW      : " + data['nat']+ print_core_network_vcn_compartment(vcn_compartment,data['compartment']))
                        
                for data in vcn['data']:
                    if 'drg_attached' in data: 
                        if data['drg_attached'] !="": print (tabs + "DRG Attached: " + data['drg_attached']+ print_core_network_vcn_compartment(vcn_compartment,data['compartment']))
                        
                for data in vcn['data']:
                    if 'local_peering' in data: 
                        for lpeer in data['local_peering']: print (tabs + "Local Peer  : "+lpeer+ print_core_network_vcn_compartment(vcn_compartment,data['compartment']))
    ...
    except oci.exceptions.RequestException: raise    
    except oci.exceptions.ServiceError: raise
...
def print_core_network_virtual_circuit(virtual_circuit):
    ...
    try:
        if len(virtual_circuit) == 0: return
...
def print_core_network_main(data):
    try:
        if 'vcn'   in data: print_core_network_vcn  (data['vcn'])
        if 'drg'   in data: print_core_network_drg  (data['drg'])
        if 'cpe'   in data: print_core_network_cpe  (data['cpe'])
        if 'ipsec' in data: print_core_network_ipsec(data['ipsec'])
        if 'remote_peering' in data: print_core_network_remote_peering(data['remote_peering'])
        if 'virtual_circuit' in data: print_core_network_virtual_circuit(data['virtual_circuit'])
...
def print_load_balancer_backendset(backendset):
    ...
    try:
        for bs in backendset:
            print ("")
            if 'backend_set' in bs: print (tabs + "backendSet : " + bs['backend_set'])
            if 'status' in bs: print (tabs + tabs + "Status : " + bs['status'] )
...
def print_load_balancer_details(load_balance_obj):
    try:
        ...
        # listeners 
        if 'listeners' in lb:
            if not  lb['listeners']: print (tabs + "Listener   : None")
...
def print_load_balancer_main(load_balancers):
    try:
        ...
        if len(load_balancers)==0: return
...
def print_file_storage_exports(exports):
    ...
    try:
        for export in exports:
            if 'path' in export:      print (tabs + "Export    : " + export['path'])
            if 'exportset' in export: print (tabs + "ExportSet : " + export['exportset'])
...
def print_file_storage_main(file_systems):
    try:
        ...
        if len(file_systems)==0: return
        print_header("File Storage",2)
            
        #print details
        for fs in file_systems:
            if 'filesystem' in fs: print (taba + fs['filesystem'])
...
def print_kms_vault_main(kms_vaults):
    try:
        ...
        if len(kms_vaults)==0: return
        print_header("KMS",2)
            
        #print details
        for vault in kms_vaults:
            if 'vault' in vault: print (taba + vault['vault'])
...
def print_database_db_system_details(dbs):
    try:
        ...
        if 'cpu_core_count' in dbs: print (tabs + "Cores   : " + str(dbs['cpu_core_count']))
        if 'node_count' in dbs: print (tabs + "Nodes   : " + str(dbs['node_count']))
        if 'version'    in dbs: print (tabs + "Version : " + dbs['version'])
        if 'host'       in dbs: print (tabs + "Host    : " + dbs['host'])
        if 'domain'     in dbs: print (tabs + "Domain  : " + dbs['domain'])
        if 'cluster_name' in dbs: print (tabs + "Cluster : " + dbs['cluster_name'])
        if 'data'       in dbs: print (tabs + "Data    : " + dbs['data'])
        if 'data_subnet' in dbs: print (tabs + "DataSub : " + dbs['data_subnet'])
        if 'backup_subnet' in dbs: print (tabs + "BackSub : " + dbs['backup_subnet'])
        if 'scan_dns' in dbs: print (tabs + "Scan DNS: " + dbs['scan_dns'])
        if 'scan_ips' in dbs: 
            for ip in dbs['scan_ips']: print (tabs + "Scan Ips: " + ip)
        if 'vip_ips' in dbs: 
            for ip in dbs['vip_ips']: print (tabs + "VIP Ips : " + ip)
        if 'listener_port' in dbs: print (tabs + "Port    : " + dbs['listener_port'])
        if 'patches' in dbs: 
            for p in dbs['patches']: print (tabs + "Patches : " + p)
    ...
    try:
       for dbs in list_db_systems:
           ...
           print_database_db_system_details(dbs)
            ...
            # db nodes
           for db_node in dbs['db_nodes']: print (tabs + db_node) 
...
def print_database_db_autonomous(dbs):
    try:
        for db in dbs:
            print (taba + db['name'])
            if 'cpu_core_count' in db: print (tabs + "Cores   : " + str(db['cpu_core_count']))
            if 'data_storage_size_in_tbs' in db: print (tabs + "Size TB : " + str(db['data_storage_size_in_tbs']))
            if 'db_name'    in db: print (tabs + "DB Name : " + db['db_name'])
            if 'time_created' in db: print (tabs + "Created : " + db['time_created'])
...
def print_database_main(list_databases):
    try:
        ...
        if len(list_databases)==0: return
...
def print_email_main(emails):
    ...
    try:
        if not 'senders' in emails and not 'supp_list' in emails: return
...
def print_core_compute_instances(instances):
    ...
    try:
        ...
        for instance in instances :
            if 'name' in instance: print (taba + instance['name'])
            if 'availability_domain' in instance: print (tabs + "AD  : " + instance['availability_domain'])
            if 'fault_domain' in instance: print (tabs + "FD  : " + instance['fault_domain'])
            if 'time_maintenance_reboot_due' in instance: print (tabs + "MRB : Maintenance Reboot Due " + instance['time_maintenance_reboot_due'])
            
            if 'image' in instance:  print (tabs + "Img : " + instance['image'])
            ...
            if 'console' in instance: print (tabs + instance['console'])
...
def print_core_compute_boot_volume_backup(backups):
    ...
    try:
        if len(backups)==0: return
    
        print_header("Boot Volume Backups",2)
        prev_id=""
        for backup in backups:
            if prev_id and prev_id!=backup['boot_volume_id']: print ("")
...
def print_core_compute_volume_backup(backups):
    ...
    try:
        ...
        prev_id=""
        for backup in backups:
            if prev_id and prev_id!=backup['volume_id']: print ("")
            print (taba + backup['name']) 
            print (tabs + tabs +"Name : "+backup['desc']) 
            print (tabs + tabs +"Size : "+backup['size']) 
...
def print_region_data(region_name, data):
    ...
    try:
        ...
        for cdata in data:
            if 'path' in cdata: print_header("Compartment "+cdata['path'],1)
            if 'network' in cdata: print_core_network_main(cdata['network'])
            if 'compute' in cdata: print_core_compute_main(cdata['compute'])
            if 'database' in cdata: print_database_main(cdata['database'])
            if 'kms' in cdata: print_kms_vault_main(cdata['kms'])
            if 'object_storage' in cdata: print_object_storage_main (cdata['object_storage'])
            if 'file_storage' in cdata: print_file_storage_main (cdata['file_storage'])
            if 'load_balancer' in cdata: print_load_balancer_main (cdata['load_balancer'])
            if 'email' in cdata: print_email_main (cdata['email'])
...
def print_showoci_data(data):
    try:
        ...
        if 'proxy'    in data:  print ("Proxy         : " + data['proxy'])
        if 'joutfile' in data:  print ("JSON Out      : " + data['joutfile'])
...
def print_oci_main(jsondata,print_version):
    try:
        for d in jsondata:
            if 'type' in d:
                if d['type']=="showoci":
                    if print_version: print_showoci_data(d['data'])
...
def summary_region_data(region_name, data):
    ...
    try:
        ...
        for cdata in data:
            ...
            compartment_header=""
            if 'path' in cdata: compartment_header="Summary - Compartment "+cdata['path']
            
            if 'compute' in cdata: summary_core_compute_main(cdata['compute'])
            if 'database' in cdata: summary_database_main(cdata['database'])
            if 'object_storage' in cdata: summary_object_storage_main (cdata['object_storage'])
            if 'file_storage' in cdata: summary_file_storage_main (cdata['file_storage'])
            if 'load_balancer' in cdata: summary_load_balancer_main (cdata['load_balancer'])
...
def get_version(cmd):
    ...
    if cmd.proxy    : data['proxy']=str(cmd.proxy)
    if cmd.joutfile : data['joutfile']=str(cmd.joutfile)
...
def get_core_network_vcn_nat(core,compartment,vcn):
    try:
        ...
        if e.code=='NotAuthorizedOrNotFound': return ""
        raise 
    except Exception as e:
...
def get_core_network_vcn_subnets(core,compartment,vcn):
    try:
        ...
        for subnet in subnets:
            ...
            if availability_domain=="None": availability_domain="Regional"
...
def get_core_network_vcn_security_rule(security_rule):
    line=""
    if type(security_rule) is oci.core.models.EgressSecurityRule:
        line="Dst: " + str(security_rule.destination).ljust(18)

    if type(security_rule) is oci.core.models.IngressSecurityRule:
        line="Src: " + str(security_rule.source).ljust(18)
        
    #protocol
    if security_rule.protocol=="1"  : line+="ICMP "
    elif security_rule.protocol=="6"  : line+="TCP  "
    elif security_rule.protocol=="17" : line+="UDP  "
    elif security_rule.protocol=="all": line+="ALL  "
    else: line+=str(" Prt(" + str(security_rule.protocol) +"), ").ljust(20)
    ...
    # Stateless
    if security_rule.is_stateless : line+=" (Stateless) "
...
def get_core_network_vcn_route_rule(core,route_rule):
    ...
    try:
        ...
        if network_dest=="privateip":
            ...
            if network_dest is None: network_dest="privateip (not exist)"
...
def get_core_network_vcn_dhcp_options_opt(dhcp_option):
    ...
    if type(opt) is oci.core.models.DhcpDnsOption:
        retstr+=str(opt.type).ljust(17) + ": " + opt.server_type
        if len(opt.custom_dns_servers)>0:
            retstr+=" - "
            for ip in opt.custom_dns_servers: retstr+=ip + "  "
            
    # if type = oci.core.models.DhcpSearchDomainOption
    if type(opt) is oci.core.models.DhcpSearchDomainOption:
        if len(opt.search_domain_names )>0:
            retstr+=str(opt.type).ljust(17) + ": "
            for ip in opt.search_domain_names : retstr+=ip + "  "
    ...
    try:
        ...
        for vcn in vcns:
            ...
            for compartment in compartments:
                ...
                if (val['igw'] or val['sgw'] or val['nat'] or val['drg_attached'] or val['local_peering']
                or val['subnets'] or val['security_lists'] or val['route_tables'] or val['dhcp_options']):
                    data.append(val)
...
def get_core_network_vnic(virtual_network, vnic_id):
    try:
        if vnic_id is None: return ""
        vnic=virtual_network.get_vnic(vnic_id).data
        returnstr= (str(vnic.private_ip) + " (Priv) ")
        if vnic.public_ip is not None: returnstr+=", "+str(vnic.public_ip) + " (Pub) "
        if vnic.skip_source_dest_check: returnstr+= ("- Skip=Y")
        if vnic.is_primary: returnstr+" - Primary "
...
def get_core_block_volume_backup_policy(block_storage, volume_id):
    ...
    try:
        ...
        if e.code=='InvalidParameter': return []
        if e.code=='TooManyRequests': return []
        raise
    except Exception as e:
...
def get_core_block_volume(block_storage, volume_id, compartment_text):
    try:
        ...
        if vol.volume_group_id is not None:
            ...
            if groupbackup is not None: groupstr+=" Backup="+groupbackup
...
def get_core_block_volume_boot_backup(block_storage,compartment):
    ...
    try:
        ...
        if len(data) > 0: return sorted(data, key=lambda k: k['name']) 
        return data
    ...
    for instance in instances :
        if instance.lifecycle_state!="TERMINATED":
            ...
            try: 
                ...
                if ocid: inst['image_id']=instance.image_id
            except: pass
            ...
            try: 
                for icc in compute.list_instance_console_connections(compartment.id, instance_id=instance.id).data:
                    if (icc.lifecycle_state=="ACTIVE"):
                        if ocid: inst['console_id']=icc.id
...
def get_core_compute_images(compute,compartment):
    ...
    try:
        ...
        for image in filtered_images:
            value={}
            value['desc']=(str(image.display_name)+ " - " + image.operating_system  + " - " 
                   + str(round(image.size_in_mbs/1024)) +"gb - Base:  " 
                   + compute.get_image(image.base_image_id).data.display_name)
            value['sum_info']='Object Storage - Images (gb)'
            value['sum_size_gb']=(str(image.size_in_mbs/1024))
            if ocid: value['id']=image.id
            data.append(value)
        return data
...
def get_database_db_nodes(database,virtual_network,dbs_id,compartment):
    ...
    try:
        ...
        for db_node in db_nodes:
            ...
            data.append("Node "+str(nodeidstr)+"  : " + str(db_node.hostname) + " - " + str(db_node.lifecycle_state) + " - " 
                   + get_core_network_vnic(virtual_network,db_node.vnic_id))
...
def get_database_db_databases(database,db_home_id,compartment):
    ...
    try:
        ...
        for db in dbs:
            ...
            pdb_name=""
            if db.pdb_name is not None: pdb_name=db.pdb_name + " - "
...
def get_database_db_homes_patches(database,db_home_id ):
    ...
    try:
        ...
        # due to exadata
        if e.code=='NotAuthorizedOrNotFound': return []
        if e.code=='IncorrectState': return []
        raise 
    except Exception as e:
...
def get_database_db_system(database,virtual_network,dbs,compartment):
    ...
    data['sum_info']='Database - '+str(dbs.shape)
...
def get_database_db_systems(database,virtual_network,compartment):
    ...
    try:
        ...
        if e.code=='NotAuthorizedOrNotFound' or e.code=='Forbidden': return []
        raise
    except Exception as e:
...
def get_database_autonomous_details(dbs):
    ...
    data['sum_info']="Autonoumous Database (OCPUs)"
...
def get_database_autonomous_transaction(database,compartment):
    ...
    except oci.exceptions.RequestException: pass    
...
def get_file_storage_mount_target(file_storage,virtual_network,export_set):
    ...
    try:
        mount_targets=oci.pagination.list_call_get_all_results(file_storage.list_mount_targets,export_set.compartment_id, export_set.availability_domain, export_set_id=export_set.id,sort_by="DISPLAYNAME").data
        data=[]
        for mt in mount_targets:
            datamt=[]
...
def get_file_storage_exports(file_storage,virtual_network,file_system_id):
    ...
    try:
        ...
        for export in exports:
            ...
            dataval['exportset']=str(export_set.display_name) + " - " + str(export_set.availability_domain ) + " - " + str(export_set.lifecycle_state ) + " - Limits: " + get_file_storage_limits(export_set)
...
def get_object_storage_preauthenticated_requests_count(object_storage,namespace_name,bucket_name):
    try:
        ...
        if e.code=='BucketNotFound': return ""
        raise 
    except Exception as e:
...
def get_object_storage_object_count(object_storage,namespace_name,bucket_name):
    try:
        ...
        if e.code=='BucketNotFound': return {'objects':"** No Permission ** ",'size':"",'sum_size_gb':"",'sum_info':""}
        raise 
    except Exception as e:
...
def get_object_storage_object_lifecycle(object_storage,namespace_name,bucket_name):
    try:
        ...
        if e.code=='LifecyclePolicyNotFound': return ""
        if e.code=='BucketNotFound': return ""
        raise 
    except Exception as e:
...
def get_object_storage_main(object_storage,compartment):
    try:
        ...
        if e.code=='NamespaceNotFound': return []
        raise
    except oci.exceptions.ServiceError: raise
    except Exception as e:
...
def get_load_balancer_details(virtual_network,load_balance_obj,status):
    try:
        ...
        for listener in lb.listeners:
            ...
            # check ssl config
            ssl_details=""
            if lo.ssl_configuration is not None: ssl_details=" - Cert: "+str(lo.ssl_configuration.certificate_name)
...
def get_email_main(email,compartment,tenancy):
    try:
        ...
        if len(senders)==0 and len(suppressions)==0: return
        ...
        if e.code=='NotAuthorized': return []
        if e.code=='NotAuthorizedOrNotFound': return []
        raise
    except Exception as e:
    ...
    try:
        ...
        for compartment in compartments:
            ...
            if cmd.all or cmd.allnoiam or cmd.network:  
                ...
                if value: data['network']=value; has_data=True
            ...
            if (region_name=="us-ashburn-1" or region_name=="us-phoenix-1"):
                if cmd.all or cmd.allnoiam or cmd.email: 
                    ...
                    if value is not None: data['email']=value; has_data=True
...
def get_oci_main_data(cmd,config):
    ...
    run_on_compartments=(
            ...
            cmd.load or cmd.email or cmd.allnoiam or cmd.kms
    )
    ...
    try:
        ...
        # Load Identity 
        identity = oci.identity.IdentityClient(config)
        if cmd.proxy : identity.base_client.session.proxies={ 'https': cmd.proxy }
        ...
        if run_on_compartments: 
            ...
            for region in identity.list_region_subscriptions(tenancy_id).data:
                ...
                if cmd.region and str(cmd.region) != str(region.region_name): continue
...
def set_parser():
    ...
    if not (result.all or result.allnoiam or result.network or result.identity or 
            ...
            result.load or result.database or result.file or result.email):
        print ("")
        print ("*************************************************************")
